...
(function($) {
	 ...
	 *				<img src="../../../../../../../pfw_set/inc/pfw/js/plugins/map/..." /><h1>Tokaichi</h1>
	 *				...end example...
	 *			</li>
	...
	GoogleMap.utils = {
		...
		onApiLoad: function() {
			...
			mapLength = ALL_MAPS.length;
			for (mapIndex = 0; mapIndex < mapLength; mapIndex++) {
				ALL_MAPS[mapIndex].init();
			}
			callbackLength = READY_CALLBACKS.length;
			for (callbackIndex = 0; callbackIndex < callbackLength; callbackIndex++) {
				try {
					READY_CALLBACKS[callbackIndex].call();
				} catch (e) {
	...
	GoogleMap.ready = function(callback) {
		...
		} else if (API_LOAD_STATUS === STATUSES.LOADING) {
